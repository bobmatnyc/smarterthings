...
class ConfigManager {
  ...
  private loadConfig(): void {
    if (!existsSync(this.envPath)) {
      console.log(chalk.yellow('⚠️  No .env.local file found. Creating new one...'));
      this.config = {};
      return;
    }
  ...
  private saveConfig(): void {
    try {
      ...
      console.log(chalk.green('✅ Configuration saved to .env.local'));
    } catch (error) {
...
class ConfigMenu {
  ...
  private displayMenu(): void {
    ...
    console.log(chalk.bold.cyan('  🏠 SmartThings MCP Server - Configuration Menu'));
  ...
  private async configureSmartThingsPAT(): Promise<void> {
    console.log('\n' + chalk.bold.blue('🔧 Configure SmartThings Personal Access Token'));
    console.log(chalk.gray('─'.repeat(60)));
    console.log();
    ...
    if (token.length < 20) {
      console.log(chalk.red('❌ Token appears too short. Please check and try again.'));
      return;
    }
  ...
  private async configureOpenRouterKey(): Promise<void> {
    console.log('\n' + chalk.bold.blue('🔧 Configure OpenRouter API Key'));
    console.log(chalk.gray('─'.repeat(60)));
    console.log();
    ...
    // Validate key format
    if (apiKey.length < 20) {
      console.log(chalk.red('❌ Key appears too short. Please check and try again.'));
      return;
    }
  ...
  private async testConnection(): Promise<void> {
    console.log('\n' + chalk.bold.blue('🔍 Testing SmartThings Connection'));
    console.log(chalk.gray('─'.repeat(60)));

    const token = this.config.get('SMARTTHINGS_PAT');

    if (!token) {
      console.log(chalk.red('❌ No SmartThings PAT configured'));
  ...
  private async clearConfiguration(): Promise<void> {
    console.log('\n' + chalk.bold.red('⚠️  Clear All Configuration'));
    console.log(chalk.gray('─'.repeat(60)));
