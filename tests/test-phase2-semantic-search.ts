#!/usr/bin/env tsx
/**
 * Phase 2: Semantic Search Testing with Real Devices
 *
 * Tests semantic search functionality with 184 real SmartThings devices
 * Focus: READ-ONLY operations, no device commands
 */
...
async function main() {
  ...
  if (!token) {
    throw new Error('SMARTTHINGS_PAT or SMARTTHINGS_TOKEN not found in environment');
  }
  ...
  try {
    ...
    // Phase 1: Load devices
    console.log('📥 PHASE 1: Loading devices from SmartThings...');
    ...
    for (const testQuery of testQueries) {
      ...
      const results = await semanticIndex.search(testQuery.query, { limit: 10 });
...
if (process.argv.includes('--expose-gc')) {
  ...
  console.log('⚠️  Running without GC exposure. For accurate memory measurements, run:');
  console.log('   bun --expose-gc test-phase2-semantic-search.ts');
  console.log();
  main();
}
